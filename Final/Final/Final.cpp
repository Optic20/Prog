...
public:
	...
	void averagegross(vector<Financial>& atu, int movieNum)
	{
		int totalGross = 0;
		for (int a = 0; a < movieNum; a++)
		{
	...
	void overAll(vector<Financial>& atu, int movieNum) {
		int over = 0;
		for (int a = 0; a < movieNum; a++) {
			if (atu[a].gross > 100) {
				over = over + 1;
	...
	void menu(vector<Financial>& atu) {
		...
		while (true) {
			...
			if (choice == 1) {
				...
				temp.averagegross(atu, movieNum);
			}
			else if (choice == 3) {
				temp.ratings(atu, movieNum);
			}
			else if (choice == 4) {
				temp.overAll(atu, movieNum);
			}
			else if (choice == 5) {
				break;
			}
